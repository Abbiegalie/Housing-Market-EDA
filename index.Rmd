...
crime_no_dup %>% group_by(`Zip Code`, `Highest Offense Description`) %>% 
    summarize(count = n()) %>% mutate(cm_crime = max(cumsum(count))) %>% arrange(desc(count))
...
``` {R}
library(gt)
crime_no_dup %>% group_by(`Zip Code`, `Location Type`) %>% summarize (count = n()) %>% gt() 
...
```{R}
library(ggplot2)
merged_data_3 %>% group_by(`Council District`) %>% summarize(average_price = mean(latestPrice)) %>% 
